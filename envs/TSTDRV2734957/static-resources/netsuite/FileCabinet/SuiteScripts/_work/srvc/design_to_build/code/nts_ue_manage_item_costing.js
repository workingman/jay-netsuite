...
function(record, runtime, search) {
	...
	/**
	 * This process is used to apply internal sales rep and external sales rep
	 * costs to sales order line items 5.1.1.5.2 Main: The process starts when
	 * the user saves a sales order
	 * 
	 * For each line item, the system performs the following: Queries the loaded
	 * cost records using the following criteria: Line item�s Item, Sales
	 * order�s sales rep, Sales order�s partner If a loaded cost record is
	 * returned, the system performs the following: Sets the line item�s
	 * internal loaded cost to the loaded cost records internal cost, Sets the
	 * line item�s external loaded cost to the loaded cost records external cost
	 * The process ends
	 */
	function apply_loaded_costing(scriptContext) {
		...
		if (!isEmpty(salesRep)) {
			for (var i = 0; i < objSalesOrder.getLineCount({
				...
				objLoadedCost = find_loaded_cost(itemId, salesRep, customer);

				if (!isEmpty(objLoadedCost)) {
					calculate_loaded_cost(objSalesOrder, objLoadedCost, i);
				}
			}
		}
	}

	function find_loaded_cost(itemId, salesRep, customer) {
		...
		if (isEmpty(sales_rep_group) || sales_rep_group.length == 0) {
			if (isEmpty(customer_group) || customer_group.length == 0) {
				loaded_cost_filters = [ [ "isinactive", "is", "F" ], "AND",
						[ "custrecord_nts_lc_sales_rep", "anyof", salesRep ],
						"AND",
						[ "custrecord_nts_lc_customer", "anyof", customer ] ]
			} else {
				loaded_cost_filters = [
						[ "isinactive", "is", "F" ],
						"AND",
				...
				loaded_cost_filters = [
						...
						[
								[ "custrecord_nts_lc_sales_rep", "anyof",
										salesRep ],
								"OR",
								[ "custrecord_nts_lc_sales_rep_group", "anyof",
										sales_rep_group[0].value ] ], "AND",
						[ "custrecord_nts_lc_customer", "anyof", customer ] ]
			} else {
				loaded_cost_filters = [
						[ "isinactive", "is", "F" ],
	...
	function calculate_loaded_cost(objSalesOrder, objLoadedCost, line) {
		...
		if (calc_method == fixed_cost) {
			loaded_cost = objLoadedCost.getValue('custrecord_nts_lc_cost_amt');
		}
