...
define(
		...
		function(record, runtime, search) {
			...
			var price_rule_create_id = runtime.getCurrentScript().getParameter(
					{
						name : 'custscript_nts_price_rule_create'
					})
			...
			function getInputData() {
				try {
					...
					var customer_method = runtime.getCurrentScript()
							.getParameter({
								name : 'custscript_nts_create_customer_method'
							})
					var customer_input = runtime.getCurrentScript()
							.getParameter({
								name : 'custscript_nts_create_customer_input'
							})
					var item_method = runtime.getCurrentScript().getParameter({
						name : 'custscript_nts_create_item_method'
					})
					var item_input = runtime.getCurrentScript().getParameter({
						name : 'custscript_nts_create_item_input'
					})
			...
			function reduce(context) {
				try {
					...
					var item;
					var price_rule_create_result = price_rule_create_search(price_rule_create_id);
					var price_rule_id;

					if (!isEmpty(price_rule_create_result)) {
						for (var i = 0; i < values.length; i++) {
							item = JSON.parse(values[i]).item
							price_rule_id = duplicate_price_rule(customer.id,
									item.id);
							if (isEmpty(price_rule_id)) {
								create_price_rule(customer.id, item.id,
			...
			function create_price_rule(customer, item, price_rule_create_result) {

				var price_rule = price_rule_create(customer, item,
						price_rule_create_result);
			...
			function price_rule_create(customer, item, price_rule_create_result) {
			...
			function price_rule_tier_create(price_rule_create_id, price_rule) {
				...
				for (var i = 0; i < price_rule_create_tier_results.length; i++) {
					...
					price_rule_tier.setValue({
						...
						value : parseFloat(price_rule_create_tier_result
								.getValue({
