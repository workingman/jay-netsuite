...
define(
		...
		function(error, record, runtime, search, nts_md_manage_price_rule,
				...
				try {
					...
					if (scriptContext.type == scriptContext.UserEventType.CREATE
						...
						var lookupFields = search.lookupFields({
							...
							id : price_rule.getValue({
								fieldId : 'custrecord_nts_alt_pr_price_rule'
							}),
							columns : [ 'custrecord_nts_pr_customer',
									'custrecord_nts_pr_item' ]
						});

						apply_name(scriptContext, price_rule,
								lookupFields.custrecord_nts_pr_item[0].value);
						apply_priority(scriptContext, price_rule);
						calculate_price(
								scriptContext,
								price_rule,
								lookupFields.custrecord_nts_pr_customer[0].value,
								lookupFields.custrecord_nts_pr_item[0].value);
					}
				...
				try {
					...
					if (generate) {
						record.submitFields({
							...
							id : price_rule.id,
							values : {
								custrecord_nts_pr_alt_generate_mb_items : false
							}
						});
						redirect.toRecord({
							type : 'customrecord_nts_alt_price_rule',
			...
			function apply_name(scriptContext, priceRule, itemId) {
				priceRule.setValue({
					fieldId : 'name',
					value : item_name_search(itemId)
				});
			}
			...
			function apply_priority(scriptContext, priceRule) {
				...
				if (price_rule_type == PR_TYPE.contract) {
					priceRule.setValue({
						fieldId : 'custrecord_nts_alt_pr_priority',
						value : 1
					});
				}
			...
			function calculate_price(scriptContext, priceRule, customerId,
					itemId) {
				var calc_details = {
					price : 0
				};
				...
				var price_rule = {
					custrecord_nts_pr_markup_percent : priceRule.getValue({
						fieldId : 'custrecord_nts_alt_pr_markup_percent'
					}),
				...
				priceRule.setValue({
					fieldId : 'custrecord_nts_alt_pr_rate',
